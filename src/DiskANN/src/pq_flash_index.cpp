...
template <typename T, typename LabelT> void PQFlashIndex<T, LabelT>::load_cache_list(std::vector<uint32_t> &node_list)
{
    ...
    {
        // v5.1 P0-3: TRUE block-aware cache — 1 IO per unique sector.
        // The round-2 version still called read_nodes() which issued one
        // AlignedRead per node internally; this v5.1 path bypasses read_nodes
        // and issues a SINGLE AlignedRead per unique sector into a shared
        // sector_buf, then uses offset_to_node() to find each node within
        // the sector. per_node_sectors is the read length in 4KB units:
        //   - 1 sector per node when _nnodes_per_sector > 0 (multi-node layout)
        //   - DIV_ROUND_UP(_max_node_len, SECTOR_LEN) sectors per node for
        //     multi-sector-node layout (each node spans multiple sectors).
        ...
        uint64_t per_node_sectors = (_nnodes_per_sector > 0)
                                        ? 1
                                        : DIV_ROUND_UP(_max_node_len, defaults::SECTOR_LEN);
        uint64_t read_len = per_node_sectors * defaults::SECTOR_LEN;
        double dedup_pct = 100.0 * (1.0 - (double)sectors.size() / (double)num_cached_nodes);
        diskann::cout << " (block-aware dedup: " << sectors.size() << " sectors for "
                      << num_cached_nodes << " nodes, " << (int)dedup_pct
                      << "% dedup) .." << std::flush;

        // Single shared sector buffer — read_len bytes, 4KB aligned.
        char *sector_buf = nullptr;
        diskann::alloc_aligned((void **)&sector_buf, read_len, defaults::SECTOR_LEN);

        // One IOContext per call (cache load is single-threaded at startup).
        ...
        for (auto sec : sectors)
        {
            ...
            // ONE AlignedRead for this entire sector (true dedup).
            AlignedRead r;
            r.offset = sec * defaults::SECTOR_LEN;
            r.len    = read_len;
            r.buf    = sector_buf;
            std::vector<AlignedRead> reqs(1, r);
            reader->read(reqs, ctx);

            // Extract all cached nodes that fall in this sector.
            for (size_t idx : idxs)
            {
                                                 ...
                                                 QueryStats *stats)
{
    ...
    while (retset.has_unexpanded_node() && num_ios < io_limit)
    {
        ...
        if (!frontier.empty())
        {
            ...
            // v5.1 P0-4: dedup frontier reads by SSD sector. Multiple ids
            // that fall in the same 4KB sector share one IO and one scratch
            // slot; the consumer uses offset_to_node() to find each id
            // within the sector. Without dedup, v4 issued one IO per id
            // even when many ids mapped to the same sector after reorder.
            std::unordered_map<uint64_t, std::vector<uint32_t>> sec_to_ids;
            std::vector<uint64_t> sec_list;
            sec_to_ids.reserve(frontier.size());
            for (auto id : frontier)
            {
                uint64_t sec = get_node_sector((size_t)id);
                if (sec_to_ids.find(sec) == sec_to_ids.end())
                {
                    sec_to_ids[sec] = {id};
                    sec_list.push_back(sec);
                }
                else
                {
            ...
            // Scratch capacity guard: sector_scratch holds
            // MAX_N_SECTOR_READS * SECTOR_LEN bytes total. Each unique sector
            // needs num_sectors_per_node * SECTOR_LEN bytes. If the dedupped
            // unique count would not fit, fall back to per-id IO.
            ...
            if (sec_list.size() > max_unique_secs)
            {
                ...
                for (auto id : frontier)
                {
                    ...
                    frontier_read_reqs.emplace_back(sec * defaults::SECTOR_LEN,
                                                    num_sectors_per_node * defaults::SECTOR_LEN, slot);
                }
                if (stats != nullptr)
                {
            ...
            {
                // DEDUP PATH: one IO per unique sector, ids in the same
                // sector share the scratch slot.
